...
export const googleReviews = [
  ...
  },
  {
    ...
    rating: 5,
    contexts: ['camping', 'stay', 'hostillam'],
  ...
  },
  {
    ...
    rating: 5,
    contexts: ['stay', 'camping', 'hostillam'],
...
function reviewsForContext(context) {
  if (!context) return googleReviews
  return googleReviews.filter((review) => (review.contexts || []).includes(context))
}
 ...
 */
export function getMonthlyGoogleReviews(date = new Date(), count = VISIBLE_COUNT, context) {
  const pool = reviewsForContext(context)
...
export function getReviewsByContext(context) {
  return reviewsForContext(context)
}
